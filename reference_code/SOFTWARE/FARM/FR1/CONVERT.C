float convertit(float,int,char);
...
void main(void)
{
    ...
    printf ("\033H\033J ENTER THE NUMBER TO BE CONVERTED: ");
    scanf  ("%f",&val);
    printf ("\n ENTER THE TABLE NUMBER OF THE CONVERSION TO BE DONE: ");
    scanf  ("%d",&num);
    printf ("\n THE RESULT IS TO BE (M)ETRIC OR (I)MPERIAL?? ");
    type=getchar();
    type=getchar();
    newval=convertit(val,num,type);
    printf ("\n\n THE CONVERTED VALUE IS :  %5.5f",newval);
    type=getchar();
    type=getchar();
}
