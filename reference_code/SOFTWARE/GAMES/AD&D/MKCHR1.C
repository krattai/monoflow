/* MKCHR1.C: Program creates and examines characters
...
void main(void)
{
	...
	while(choice != '3')
	{
		...
		switch(choice)
		{
			case '1':	file.ff_reserved[0] = NULL;
						fini = findfirst("*.dat",&file,0);
						ypos = 0;
						while(!fini)
						{
							x=0;
							while(file.ff_name[x] != '.')
							{
								chrs[ypos][x] = file.ff_name[x];
								x++;
							}
							chrs[ypos][x] = NULL;
							ypos++;
						...
						{
							Put_window(1,2,80,24,WHITE,BLUE);
							CLS;
							display(chr[charnum]);
						}
						getch();
